...
| Som medication feedback: actual medicine must be clear. | One-med medication sessions should name the medicine, not only the category. | Today next session, smoke test first-run onboarding. |
| Som asked how caregiver knows medication was administered. | Patient action logging is explicit but honest: Done means patient confirmed, Help/Skip are logged, and CueGuide does not claim physical swallowing proof. | Live Session, Alert Feed, Reports, confirmation-limit note. |
...
| Source | Product Translation | Current App Surface |
| --- | --- | --- |
| [Alzheimer's Association 2026 Facts and Figures](https://www.alz.org/alzheimers-dementia/facts-figures): an estimated 7.4 million Americans 65+ live with Alzheimer's, nearly 13 million unpaid caregivers provide care, and care costs are projected at $409B in 2026. | The opportunity is large, but credibility depends on caregiver utility, not generic reminders. | Today, Alerts, Reports, Settings data readiness. |
...
| [Hero caregiver page](https://herohealth.com/caregivers/) and [Hero help docs](https://support.herohealth.com/en/articles/8058271-how-does-the-hero-dispense-medications): hardware dispenser plus caregiver app, missed-dose, late-dose, and low-supply notifications. | Hardware competitors can better prove dispensing; CueGuide must stay honest that `Done` is patient confirmation, not physical administration proof. | Live Session language, Reports interpretation, Settings caveat. |
| [HHS mobile health app guidance](https://www.hhs.gov/hipaa/for-professionals/special-topics/health-apps/index.html) and [FTC mobile health app best practices](https://www.ftc.gov/business-guidance/resources/mobile-health-app-developers-ftc-best-practices) | Health app trust depends on data minimization, clear privacy claims, and security-by-design, not just an attractive UI. | Server-only provider keys, local backup export, privacy route, Supabase proof caveat. |
